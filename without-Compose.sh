# Without-Compose.sh Prototype Script, still in development as long as Docker-compose issue "ContainerConfig" is not solved

# clean docker env from old containers if existing and build images & NW
...
#
#
read -p "Everything okay? Lets Continue with Database setup, press enter key.." REPLY
# DATABASE Container Run & Config
# TODO introduce secure password management either here or directly in orchestrated setup
...
#
# Master-to-DB Init, table schema
read -p "Everything okay? Lets Continue with Master-to-DB Init, setting table schema. press enter key.." REPLY
# TODO SSL config for DB & WEB connections, enforce SSL
...
#
#
read -p "Everything okay? Lets Continue with the MASTER NODE WIZARDS, Zones and Api configs, press enter.."   REPLY
# TODO automate node wizard via "node setup"- based script
echo "Master1 Wizard"
...
#
#
read -p "Everything okay? Lets Continue with the Web config, press enter key.."  REPLY
#
#
echo "Web config"
...
echo "Token for Web Setup:"
echo ./web/token.txt
echo -p "Go to 172.18.0.12/icingaweb2/setup and follow the wizard, inserting the tokey you see above" REPLY
#
#
read -p "Lets compare & assert correct config across masters.. "  REPLY
docker exec -it master1 /bin/bash -c 'icinga2 daemon -C'
docker exec -it master2 /bin/bash -c 'icinga2 daemon -C'
#
#
#
echo -p " lets config the sonde. Do you need info first? (y= prints info)" REPLY
if [ REPLY == "y" ];
    then
...
echo -e -p "if you want to backup web-container and db-container and mysql-data-dir, as their state is inconsistent related to this script, after manual input in the Web User interface. Execute manually the following: \n
